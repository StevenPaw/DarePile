<template>
  <div class="game-view">
    <header class="game-header">
      <button class="btn btn--ghost btn--sm" @click="router.push({ name: 'stats', params: { hash } })">
        📊 Statistiken
      </button>
      ...
      <div class="game-header__actions">
        <button class="btn btn--ghost btn--sm" :disabled="reshuffling" @click="onReshuffle" title="Stapel neu mischen">
          🔀
        </button>
        <button class="btn btn--ghost btn--sm" @click="router.push({ name: 'setup-cards', params: { hash }, query: { edit: 'true' } })" title="Karten bearbeiten">
          ✏️
        </button>
        <button class="btn btn--ghost btn--sm" @click="showAddCard = true">
          + Karte
        </button>
      </div>
    </header>
    ...
    <div class="game-area" @click.self="skipCard">
      ...
      <div v-if="availableCards.length === 0 && !drawnCard" class="all-done">
        ...
        <button class="btn btn--primary" style="margin-top:0.5rem" @click="router.push({ name: 'stats', params: { hash } })">
          Statistiken ansehen
        </button>
      </div>
    </div>
